...
type FundScope = { userId: bigint; fundProductIds: bigint[] };

@Injectable()
export class FundPostsService {
  constructor(private readonly prisma: PrismaService) {}

  private async assertFundUser(userId: string): Promise<FundScope> {
    const user = await this.prisma.user.findUnique({ where: { id: BigInt(userId) }, select: { id: true, role: true, status: true, fundProductId: true } });
    if (!user || user.role !== 'FUND' || user.status !== 'ACTIVE') throw new UnauthorizedException('仅基金角色可维护帖子');
    if (!user.fundProductId) throw new UnauthorizedException('当前基金账号未绑定基金');
    const organizationRows = await this.prisma.task.findMany({ where: { fundProductId: user.fundProductId }, select: { organizationId: true }, distinct: ['organizationId'] });
    const organizationIds = organizationRows.map((row) => row.organizationId);
    const productRows = organizationIds.length
      ? await this.prisma.task.findMany({ where: { organizationId: { in: organizationIds }, fundProductId: { not: null } }, select: { fundProductId: true }, distinct: ['fundProductId'] })
      : [];
    const fundProductIds = [...new Set([user.fundProductId, ...productRows.flatMap((row) => row.fundProductId ? [row.fundProductId] : [])])];
    return { userId: user.id, fundProductIds };
  }

  private async assertScope(userId: string | undefined, role: string | undefined): Promise<FundScope | null> {
    if (!userId) throw new UnauthorizedException('登录状态无效');
    if (role === 'OPERATOR') return null;
    return this.assertFundUser(userId);
  }

  async list(fundProductId: string, userId?: string, role?: string) {
    const scope = await this.assertScope(userId, role);
    const where = role === 'OPERATOR'
      ? { fundProductId: BigInt(fundProductId), status: 'ACTIVE' }
      : { status: 'ACTIVE', OR: [{ fundProductId: { in: scope!.fundProductIds } }, { createdBy: scope!.userId }] };
    const rows = await this.prisma.fundTask.findMany({ where, include: { posts: { where: { status: 'ACTIVE' }, orderBy: { id: 'asc' } } }, orderBy: [{ taskName: 'asc' }, { updatedAt: 'desc' }] });
    return rows.map((row) => this.mapTask(row));
  }

  async progress(userId: string, fundProductId: string) {
    const scope = await this.assertFundUser(userId);
    const rows = await this.prisma.fundTask.findMany({
      where: { status: 'ACTIVE', OR: [{ fundProductId: { in: scope.fundProductIds } }, { createdBy: scope.userId }] },
      include: {
    ...
    return rows.map((row) => {
      ...
      const pendingSubmitCount = row.tasks.reduce((sum, task) => sum + task.claims.filter((claim) => claim.status === 'PENDING_SUBMIT').length, 0);
      const pendingReviewCount = row.tasks.reduce((sum, task) => sum + task.claims.filter((claim) => claim.status === 'PENDING_REVIEW').length, 0);
      const reworkingCount = row.tasks.reduce((sum, task) => sum + task.claims.filter((claim) => claim.status === 'REWORKING').length, 0);
      const submittedCount = row.tasks.reduce((sum, task) => sum + task.claims.filter((claim) => ['PENDING_REVIEW', 'APPROVED'].includes(claim.status)).length, 0);
      return { id: row.id.toString(), taskName: row.taskName, platform: row.platform, postCount, publishedTaskCount, claimedCount, pendingSubmitCount, submittedCount, pendingReviewCount, reworkingCount, approvedCount, completionRate: postCount ? Math.round((approvedCount / postCount) * 100) : 0, status: row.tasks.some((task) => task.status === 'IN_PROGRESS') ? 'IN_PROGRESS' : row.tasks[0]?.status ?? 'NOT_PUBLISHED' };
    });
  }

  async create(userId: string, fundProductId: string, input: UpsertFundPostDto) {
    const scope = await this.assertFundUser(userId);
    if (!scope.fundProductIds.some((id) => id === BigInt(fundProductId))) throw new UnauthorizedException('当前基金账号无权使用该基金产品');
  ...
  async update(userId: string, id: string, input: UpsertFundPostDto) {
    ...
    if (!scope.fundProductIds.some((productId) => productId === existing.fundProductId)) throw new UnauthorizedException('当前基金账号无权修改该任务');
