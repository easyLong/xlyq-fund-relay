...
export class SessionGuard implements CanActivate {
  ...
  private authorize(method: string, path: string, user: SessionUser, request: { params?: Record<string, string>; query?: Record<string, string>; body?: Record<string, unknown> }) {
    ...
    if (requestedUserId && requestedUserId !== user.id) throw new UnauthorizedException('无权操作其他用户的数据');

    const viewerId = request.query?.viewerId;
    const viewerRole = request.query?.viewerRole;
    if (viewerId && viewerId !== user.id) throw new UnauthorizedException('无权查看其他用户的任务详情');
    if (viewerRole && viewerRole !== (user.role === 'OPERATOR' ? 'operator' : user.role === 'EXECUTOR' ? 'executor' : 'fund')) throw new UnauthorizedException('无权使用其他角色查看数据');
    ...
    if (path.includes('/fund-posts')) {
      ...
      if (method === 'GET' && path.endsWith('/progress') && user.role !== 'FUND') throw new UnauthorizedException('仅基金角色可查看进度');
      if (method === 'PUT' && typeof request.query?.userId === 'string' && request.query.userId !== user.id) throw new UnauthorizedException('无权修改其他用户的帖子');
    }
    if (path.endsWith('/tasks') && !path.includes('/users/') && method === 'GET' && user.role !== 'OPERATOR') throw new UnauthorizedException('仅运营角色可查看全部任务');
    if (/\/tasks\/\d+$/.test(path) && method === 'GET' && !['OPERATOR', 'EXECUTOR'].includes(user.role)) throw new UnauthorizedException('仅运营或兼职角色可查看任务详情');
    if (path.includes('/dashboards/operator') || path.endsWith('/tasks') && method === 'POST' || path.includes('/publish') || path.includes('/unpublish') || path.includes('/review') || path.includes('/remind')) {
      if (user.role !== 'OPERATOR') throw new UnauthorizedException('仅运营角色可执行该操作');
    }
    if (path.includes('/task-market') || path.includes('/task-submissions') && method !== 'POST' || path.includes('/claims')) {
      if (user.role !== 'EXECUTOR') throw new UnauthorizedException('仅兼职角色可执行该操作');
    }
    if (path.includes('/dashboards/fund') && user.role !== 'FUND') throw new UnauthorizedException('仅基金角色可查看该看板');
    if (method === 'POST' && path.includes('/tasks/') && path.endsWith('/claims') && bodyUserId !== user.id) throw new UnauthorizedException('无权代替其他兼职领取任务');
    if ((bodyUserId && ['POST', 'PUT'].includes(method)) && user.role !== 'EXECUTOR' && path.includes('/task-submissions')) throw new UnauthorizedException('无权提交其他用户的任务结果');
    ...
    if ((reviewerId ?? operatorId) && (reviewerId ?? operatorId) !== user.id) throw new UnauthorizedException('无权使用其他运营账号操作');
  }
}
